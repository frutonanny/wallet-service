...
Это сервис для работы с балансом пользователей. Разработан в
...
Сервис предусматривает следующие методы:
...
6. Написаны интеграционные и юнит-тесты. ???
...
2. Для оплаты заказа покупки услуги вначале резервируются деньги на счету при помощи метода **reserve**, а затем
   вызывается метод списания средств **write-off**. Без первоначального резервирования, списание средств не
   предусмотрено.
3. Так как метод списания (по заданию) на вход получает те же параметры, что и при резервировании, то есть опасность в
   ...
   один идентификатор (пользователя, заказа, сервиса) не совпадает. Если отличается только сумма, то здесь 2 решения:
...
- если новая сумма меньше зарезервированной, то списываем переданную сумму с резерва, а разницу в виде ранее
  зарезервированной и новой суммой возвращаем на баланс (**смотри п.5 в сценарии тестирования**).

4. В сервисе предусмотрено 2 метода получения списка транзакций, отличающиеся видами пагинаций:
